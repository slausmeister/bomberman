...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    # (s, a, r, s')
        ...
        if len(self.transitions)==TRANSITION_HISTORY_SIZE:
            ...
            for exp in exps:
                train(exp,self,self_action)
        

def train(experience,self,self_action):
    beta = self.model
    #print(experience[0])
    ...
    for i in range(len(ACTIONS)):
        features = experience[3]
        beta_best.append(features@beta[i])

    q_max = np.amax(beta_best)
    delta = experience[2] + gamma * q_max - experience[0]@beta[index]
    
    #print(new_game_state['round'])
    for i in range(len(beta[index])):
        beta[index][i] = beta[index][i]+alpha*delta*experience[0][i]
